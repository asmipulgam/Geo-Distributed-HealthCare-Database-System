...
import OdImage from "../assets/OrganDonar.png";
...
export default function OrganSearch() {
  ...
  return (
    <div
      className="min-h-screen p-6 bg-no-repeat bg-center bg-contain relative flex justify-center items-start"
      style={{ backgroundImage: `url(${OdImage})` }}
    >
      {/* Dark Overlay */}
      ...
      <div className="relative z-10 w-full flex justify-center pt-10">
        <div className="max-w-4xl w-full bg-white/90 backdrop-blur-xl p-6 rounded-xl shadow-2xl border">
          <h1 className="text-xl font-bold mb-4 text-center">
            Organ Donation — Find Potential Donors
          </h1>
          ...
          <form onSubmit={handleSearch} className="space-y-3">
            ...
            <div className="flex gap-2 justify-center">
              ...
              <button type="button" className="px-6 py-2 bg-gray-200 rounded" onClick={() => { setResults([]); setError(null); }}>
                Clear
              </button>
            </div>
          </form>
                ...
                <tbody>
                  {results.map((r, idx) => (
                    <tr key={r.Patient_ID || idx} className="border-t">
                      ...
                      <td className="px-2 py-1">{r.distance_km ? `${Number(r.distance_km).toFixed(2)} km` : ''}</td>
